        ...
        for c, i in enumerate(range(1, len(excel[column]), 7)):
            ...
            for j in range(len(lessons)):
                if lessons[j] is not None:
                    sp = lessons[j].split('\n')
                    # TODO(Ваня): возможно нужно делать кабинеты None
                    if len(sp) == 2:
                        sl = {'lesson': sp[0].strip(), 'teacher': sp[1].strip()}
                    elif len(sp) == 3:
                        sl = {'lesson': ' '.join(sp[:2]).strip(), 'teacher': sp[2].strip()}
                    else:
                        st = sp[0]
                        if st == 'Русский язык Петковская В.Н.':
                            sl = {'lesson': 'Русский язык', 'teacher': 'Петковская В.Н.'}
                        elif st == 'Математика Петковская В.Н.':
                            sl = {'lesson': 'Математика', 'teacher': 'Петковская В.Н.'}
                        elif st == 'Хочу всё знать Котлярова Я.Ю ДО':
                            sl = {'lesson': 'Хочу всё знать', 'teacher': 'Котлярова Я.Ю'}
                        elif st == 'Физическая культура':
                            sl = {'lesson': 'Физическая культура', 'teacher': 'Неизвестно'}
                        elif st == 'Музыка Котлярова Я.Ю.':
                            sl = {'lesson': 'Музыка', 'teacher': 'Котлярова Я.Ю.'}
                        elif st == 'расписание':
                            sl = {'lesson': 'расписание', 'teacher': 'Неизвестно'}
                        else:
                            sl = {'lesson': st, 'teacher': 'Неизвестно'}
                    lessons[j] = sl
            day_schedule = {}
