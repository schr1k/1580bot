...
def load_excel_schedule(link: str):
    df = pd.read_excel(link, header=None).T.values.tolist()
    with open(f'excel.json', 'w', encoding='utf-8') as f:
        json.dump(df, f, indent=4, ensure_ascii=False, ignore_nan=True)


def make_excel_schedule():
...
def load_google_schedule(link: str):
    df = pd.read_excel(link, header=0).T.values.tolist()
    with open(f'excel.json', 'w', encoding='utf-8') as f:
        json.dump(df, f, indent=4, ensure_ascii=False, ignore_nan=True)


def make_google_schedule():
