...
def make_schedule_4h():
    ...
    with open(f'{config.PROJECT_PATH}/src/four/high/excel.json', 'w', encoding='utf-8') as f:
        json.dump(df, f, indent=4, ensure_ascii=False, ignore_nan=True)

    with open(f'{config.PROJECT_PATH}/src/four/high/excel.json', encoding='utf-8') as f:
        excel = json.load(f)
